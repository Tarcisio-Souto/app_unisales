<template>
  <layout>
    ...
    <div class="table-responsive-lg" id="div_table">
      ...
      <pagination
        :data="this.form.categorias"
        v-bind:showDisabled="true"
        icon="chevron"
        v-on:change-page="list"
      ></pagination>
...
export default {
  ...
  methods: {
    async list(page = 1) {
      await axios
        .get(`/categorias/listar-todos?page=${page}`)
        .then(({ data }) => {
          this.form.categorias = data;
        })
        .catch(({ response }) => {
  ...
  computed: {
    sortedCategorias: function () {
      ...
      for (var i = 0; i < this.form.categorias.total; i++) {
        //console.log(this.form.categorias.data[i]);
        this.form.arrCategorias.push(this.form.categorias.data[i]);
        
      }
      console.log(this.form.arrCategorias);
