<template>
  <layout>
    ...
    <form
      @submit.prevent="cadPosition"
      enctype="multipart/form-data"
      id="formAddList"
    >
      <div class="row">
        <div class="col-md-8">
          <label for="inputName" style="font-weight: bold">Cargo</label
          ><br /><br />
    ...
    <b-container fluid>
      ...
      >
        ...
        <template #cell(id)="row">
          <Link :href="'/cargo/visualizar/' + row.value"
            ><i class="fas fa-eye"></i
          ></Link>
          <Link :href="'/cargo/editar/' + row.value"
            ><i class="fas fa-edit"></i
          ></Link>
          <span
            ><i class="fas fa-trash-alt" @click="delPosition(row.value)"></i
          ></span>          
        </template>
...
export default {
  ...
  created() {
    axios.get("/cargos/listar-todos").then((response) => {
  ...
  methods: {
    ...
    cadPosition() {
      axios.post("/cargo/registrar", this.form).then(
        function (res) {
          if (res.data["success"]) {
            bootbox.alert({
              centerVertical: true,
    ...
    delPosition(id) {
      ...
      bootbox.confirm({
        ...
        callback: function (result) {
          if (result == true) {
            axios.post("/cargo/deletar/" + id).then(
              function (res) {
                if (res.data["success"]) {
                  bootbox.alert({
                    centerVertical: true,
